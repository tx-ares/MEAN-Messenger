...
router.post('/signin', function(req, res, next) { // Signin route
    User.findOne({email: req.body.email} , function(err, user) {
        ...
        var token = jwt.sign({user: user}, 'secret', {expiresIn: 7200}); // With this method , we can create a token for the authorized session, by passing in the 'user' object we got from the db, a 'secret' string, and the expiration timer.  7200 seconds is 2 hours.
        res.status(200).json({
